...
function exp_array = expand(node_x, node_y, xTarget, yTarget, OBSTACLE, MAX_X, MAX_Y)
    exp_array = []; % children nodes
    exp_count = 1;
    c2 = size(OBSTACLE, 1);
    for k = 1 : -1 : -1 % explore surrounding locations
        for j = 1 : -1 : -1
            if ((k ~= j || k ~= 0) &&((k+j) ~=0) && (k ~= j))  % the node itself is not its successor
                s_x = node_x + k;
                s_y = node_y + j;
